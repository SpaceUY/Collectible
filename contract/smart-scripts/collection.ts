...
collection
  ...
  .action(async (communityId, collectionName, collectionAddress) => {
    ...
    // Validate collectionAddress
    if (typeof collectionAddress !== "string" || collectionAddress.length === 0) {
      console.error("Invalid collection address. Please provide a valid string.");
      process.exit(1);
    }
